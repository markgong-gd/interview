...
HTTP/2“基本上”解决对头阻塞，而不是“完全”。

HTTP/2虽然使用“帧”，“流”，“多路复用”，没用“对头阻塞”，但这些都是在应用层，在下层TCP协议上，还是会发生“对头阻塞”。

在HTTP/2把多个“请求-应答”分解成流，交给TCP后，TCP会在拆成更小的包一次发送。网络较差时，可能出现**丢包**，TCP为了保证可靠传输，有个特别的“**丢包重传**”机制，丢失的包必须等待重新传输确认，其它的包即使已经收到，也只能放在缓存去里，上层应用拿不到。

![](https://static001.geekbang.org/resource/image/d2/03/d263202e431c84db0fd6c7e6b1980f03.png?wh=1235*545)

> QUIC协议

QUIC选定UDP，在它之上把TCP的那一套连接管理、拥塞窗口、流量控制等“搬”过来，打造出一个全新的可靠传输协议.

> QUIC的特点

QUIC基于UDP，由于UDP是“无连接”的，不需要“握手”和“挥手”，所以比TCP要快。

QUIC基于UDP实现了可靠传输，保证数据一定能够抵达目的地。引用HTTP/2的“流”、“多路复用”，单个“流”是有序的，可能会因为丢包阻塞，但不影响其它“流”。

为了防止网络上中间设备识别协议的细节，QUIC全面采用加密通信。直接使用TLS1.3，也获得了0-RTT（第一个数据包就可以携带有效的业务数据）、1-RTT连接好处。

> QUIC内部细节

不在是ip地址 + 端口标记两端，采用**连接ID**，这样即使从一个网络换到另外一个网络也不需要断开、重新连接，连接可以直接**迁移**。例如：数据流量回到家后自动切换到wifi，网络发生变化，使用连接ID标记，两端连接不需要变化。

![QUIC单个连接组成](https://static001.geekbang.org/resource/image/ae/3b/ae0c482ea0c3b8ebc71924b19feb9b3b.png?wh=1237*715)

![QUIC一个流组成](https://static001.geekbang.org/resource/image/9a/10/9ab3858bf918dffafa275c400d78d910.png?wh=1270*619)

> HTTP/3协议

由于QUIC已经支持加密、流、多路复用，所以HTTP/3把流控制都交给QUIC去做，调用QUIC函数。

![HTTP/3帧](https://static001.geekbang.org/resource/image/26/5b/2606cbaa1a2e606a3640cc1825f5605b.png?wh=1262*430)

> HTTP/3服务发现

HTTP/3不在是写死端口号，怎样识别HTTP/3？

浏览器通过HTTP/2连接服务器，然后服务器启用HTTP/2连接后发送一个**Alt-Svc**帧，包含一个“h3-host:port”的字符串，告诉浏览器在另一个端点上提供等价的HTTP/3服务。
...
- 完全保持了与HTTP/1的兼容
- 在安全上，对HTTPS各方面做了强化，TLS1.2+，而且只能使用前向安全的密码套件（即 ECDHE），这同时也就默认实现了“TLS False Start”，支持 1-RTT 握手，所以不需要再加额外的配置就可以自动实现 HTTPS 加速。
- 提高带宽利用率，HTTP/1在上行时，下行空闲；下行时，上行空闲，HTTP/2在上下行都有流在传输数据。
- 流也会存在依赖关系，可能会存在等待导致的阻塞，这就是“延迟”，HTTP/2可以标记流的“**优先级**”，给资源标上传输优先级，提高页面首屏展示。

> HTTP/2缺点

- 在TCP层任然存在“对头阻塞”问题。
- 切换网络需要重新建立连接，要再次“握手”，HPACK字典丢失，需要重新结算。
- HTTP/2只开一个连接，一旦这个连接出现问题，会影响整个网站，HTTP/1虽然慢，但会开多个连接，一两个连接出现问题，不会影响到整个网站。
...
客户端在发起“Client Hello”时，后面会带上“ALPN”扩展。

![](https://static001.geekbang.org/resource/image/d8/b0/d8f8606948bbd63c31466e464c1956b0.png?wh=1233*545)

![](https://static001.geekbang.org/resource/image/19/a7/19be1138574589458c96040e1a23b3a7.png?wh=1197*361)
